...
export const DisplayPreview: React.FC<{
  ...
  const frameIdToTournamentId = (frameId: string) => {
    if (frameId.startsWith('prev-exp-')) return frameId.slice('prev-exp-'.length);
    if (frameId.startsWith('prev-')) return frameId.slice('prev-'.length);
    return frameId;
  };
  ...
  const reloadFrame = (id: string) => {
    ...
    if (!iframe) return;
    const key = frameIdToTournamentId(id);
    setLoaded(prev => ({ ...prev, [key]: false }));
    const s = iframe.src;
                  ...
                  <div className="ms-auto d-flex align-items-center flex-wrap gap-2">
                    ...
                    <div className="position-relative">
                      <div className="d-flex align-items-center gap-2">
                        <span className="badge text-bg-dark d-none d-md-inline">{fitLabel}</span>
                        <div className="btn-group btn-group-sm" role="group" aria-label="HUD y expansión">
                          <button
                      ...
                      {showHudMenuHere && (
                        ...
                        >
                          <div className="hud-config-menu__section">
                            <div className="hud-config-menu__label">Ajuste</div>
                            <div className="btn-group btn-group-sm w-100" role="group" aria-label="Modo de ajuste">
                              <button
                                type="button"
                                className={`btn ${fit === 'contain' ? 'btn-secondary' : 'btn-outline-secondary'}`}
                                onClick={() => setFitMode(t.id, 'contain')}
                              >
                                Contener
                              </button>
                              <button
                                type="button"
                                className={`btn ${fit === 'width' ? 'btn-secondary' : 'btn-outline-secondary'}`}
                                onClick={() => setFitMode(t.id, 'width')}
                              >
                                Ancho
                              </button>
                              <button
                                type="button"
                                className={`btn ${fit === 'height' ? 'btn-secondary' : 'btn-outline-secondary'}`}
                                onClick={() => setFitMode(t.id, 'height')}
                              >
                                Alto
                              </button>
                            </div>
                          ...
                          <div className="hud-config-menu__section">
                            <div className="hud-config-menu__label">Tema y efectos</div>
                            <div className="btn-group btn-group-sm w-100" role="group" aria-label="Tema y efectos">
                              <button
                                type="button"
                                className={`btn ${(t.displayTheme || 'dark') === 'light' ? 'btn-warning' : 'btn-outline-warning'}`}
                                onClick={() => onToggleTheme(t.id)}
                              >
                                {(t.displayTheme || 'dark') === 'light' ? '🌞 Claro' : '🌙 Oscuro'}
                              </button>
                              <button
                                type="button"
                                className={`btn ${isFxOn(t.id) ? 'btn-outline-warning' : 'btn-warning'}`}
                                onClick={() => toggleFx(t.id)}
                              >
                                {isFxOn(t.id) ? '✨ FX activos' : '🚫 Sin FX'}
                              </button>
                            </div>
                          </div>
                          <div className="hud-config-menu__section">
            ...
            >
              ...
              <div className="d-flex align-items-center gap-2 mb-2 flex-wrap">
                ...
                <div className="position-relative">
                  <div className="d-flex align-items-center gap-2">
                    <span className="badge text-bg-dark d-none d-md-inline">{fitLabel}</span>
                    <div className="btn-group btn-group-sm" role="group" aria-label="HUD">
                      <button
                  ...
                  {overlayHudActive && (
                    ...
                    >
                      <div className="hud-config-menu__section">
                        <div className="hud-config-menu__label">Ajuste</div>
                        <div className="btn-group btn-group-sm w-100" role="group" aria-label="Modo de ajuste">
                          <button type="button" className={`btn ${fit === 'contain' ? 'btn-secondary' : 'btn-outline-secondary'}`} onClick={() => setFitMode(t.id, 'contain')}>
                            Contener
                          </button>
                          <button type="button" className={`btn ${fit === 'width' ? 'btn-secondary' : 'btn-outline-secondary'}`} onClick={() => setFitMode(t.id, 'width')}>
                            Ancho
                          </button>
                          <button type="button" className={`btn ${fit === 'height' ? 'btn-secondary' : 'btn-outline-secondary'}`} onClick={() => setFitMode(t.id, 'height')}>
                            Alto
                          </button>
                        </div>
                      </div>
                      <div className="hud-config-menu__section">
                        <div className="hud-config-menu__label">Tema y efectos</div>
                        <div className="btn-group btn-group-sm w-100" role="group" aria-label="Tema y efectos">
                          <button type="button" className={`btn ${(t.displayTheme || 'dark') === 'light' ? 'btn-warning' : 'btn-outline-warning'}`} onClick={() => onToggleTheme(t.id)}>
                            {(t.displayTheme || 'dark') === 'light' ? '🌞 Claro' : '🌙 Oscuro'}
                          </button>
                          <button type="button" className={`btn ${isFxOn(t.id) ? 'btn-outline-warning' : 'btn-warning'}`} onClick={() => toggleFx(t.id)}>
                            {isFxOn(t.id) ? '✨ FX activos' : '🚫 Sin FX'}
                          </button>
                        </div>
                      </div>
